...
if (isset($_POST['submit'])) {
    ...
    if ($check_row['count'] > 0) {
        ...
        echo "<script>alert('Sorry for the inconvinence, but we are fully booked as of now. Please select another date and time.Thank you !!!'); window.location='admin_welcome.php';</script>";
    } else {
        ...
        if (mysqli_query($conn, $query)) {
            ...
            echo "<script>alert('Appointment booked successfully!'); window.location='admin_welcome.php';</script>";
        } else {
            // If there is an error in the query, show an error message
            echo "<script>alert('Error in booking appointment: " . mysqli_error($conn) . "'); window.location='admin_welcome.php'; </script>";
        }
    }
